...
public:
	NumMatrix(vector<vector<int>> matrix) {
		...
		for (int i=1; i<=n; i++){
			for (int j=1; j<=m; j++){
				partialSum[i][j] = 
					partialSum[i-1][j] + partialSum[i][j-1] - 
	...
	int sumRegion(int row1, int col1, int row2, int col2) {
		if (partialSum.size() == 0) { return 0; }
		return partialSum[row2+1][col2+1] 
			- partialSum[row2+1][col1] 
			- partialSum[row1][col2+1] 
			+ partialSum[row1][col1];
	}
};
