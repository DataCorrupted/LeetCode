class Solution{
	...
	int binarySearch(int target, int l, int r, bool (Solution::*cmpFunc)(int, int)){
...
public:
	bool searchMatrix(vector<vector<int>>& matrix, int target) {
		if (matrix.size() == 0 || matrix.back().size() == 0) { return false; }
		m = &matrix;
		int idx = binarySearch(target, 0, m->size() * m->back().size() - 1, &Solution::le);
