 ...
 private:
  ...
  void greedyPickWords() {
    ...
    do {
      ...
      m_idx++;
    } while (m_idx != m_words.size() &&
             charCnt + m_words[m_idx].size() + 1 <= m_maxWidth);
  }
...
REGISTER_TEST(0) {
  std::vector<string> v = {"This",          "is", "an", "example", "of", "text",
                           "justification."};
  std::vector<string> groundTruth = {"This    is    an", "example  of text",
                                     "justification.  "};
  int maxWidth = 16;
...
REGISTER_TEST(1) {
  std::vector<string> v = {"What",           "must",  "be",
                           "acknowledgment", "shall", "be"};
  std::vector<string> groundTruth = {"What   must   be", "acknowledgment  ",
                                     "shall be        "};
  int maxWidth = 16;
  auto result = Solution().fullJustify(v, maxWidth);
  return result == groundTruth;
...
REGISTER_TEST(3) {
  std::vector<string> v = {"Don't", "go",    "around", "saying", "the",
