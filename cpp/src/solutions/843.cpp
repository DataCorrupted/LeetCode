...
class Master {
 ...
 public:
  Master(vector<string> wordlist, string secret)
      : wordlist(wordlist), secret(secret), num_guess(0) {}
  int guess(string& word) {
    this->num_guess++;
    MATCH(secret, word);
    return match;
  }
  bool successful() { return this->num_guess <= 10; }
};
#endif
 ...
 public:
  void findSecretWord(vector<string>& wordlist, Master& master) {
    ...
    for (int x = 0; x < 20; x++) {
      ...
      for (size_t j = 0; j < len; j++) {
        // Since matrix[max_idx][max_idx] == 6, it will be labled as impossible
        // too.
...
REGISTER_TEST(example2) {
  ...
  vector<string> wordlist = {
      "mjpsce", "giwiyk", "slbnia", "pullbr", "ezvczd", "dwkrmt", "qgzebh",
      "wvhhlm", "kqbmny", "zpvrkz", "pdwxvy", "gilywa", "gmrrdc", "vvqvla",
      "rmjirt", "qmvykq", "mhbmuq", "unplzn", "qkcied", "eignxg", "fbfgng",
      "xpizga", "twubzr", "nnfaxr", "skknhe", "twautl", "nglrst", "mibyks",
      "qrbmpx", "ukgjkq", "mhxxfb", "deggal", "bwpvsp", "uirtak", "tqkzfk",
      "hfzawa", "jahjgn", "mteyut", "jzbqbv", "ttddtf", "auuwgn", "untihn",
      "gbhnog", "zowaol", "feitjl", "omtiur", "kwdsgx", "tggcqq", "qachdn",
      "dixtat", "hcsvbw", "chduyy", "gpdtft", "bjxzky", "uvvvko", "jzcpiv",
      "gtyjau", "unsmok", "vfcmhc", "hvxnut", "orlwku", "ejllrv", "jbrskt",
      "xnxxdi", "rfreiv", "njbvwj", "pkydxy", "jksiwj", "iaembk", "pyqdip",
      "exkykx", "uxgecc", "khzqgy", "dehkbu", "ahplng", "jomiik", "nmcsfe",
      "bclcbp", "xfiefi", "soiwde", "tcjkjp", "wervlz", "dcthgv", "hwwghe",
      "hdlkll", "dpzoxb", "mpiviy", "cprcwo", "molttv", "dwjtdp", "qiilsr",
      "dbnaxs", "dbozaw", "webcyp", "vftnkr", "iurlzf", "giqcfc", "pcghoi",
      "zupyzn", "xckegy"};
  Master master(wordlist, secret);
  Solution().findSecretWord(wordlist, master);
  return master.successful();
