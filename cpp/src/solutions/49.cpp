class Solution {
 private:
  ...
  std::vector<unsigned> primeNum = {
      2731, 8161,  9679,  6229, 11519, 17293, 3517, 5273,  6449,
      6481, 10079, 12547, 4007, 6679,  2351,  6763, 14629, 13147,
      5591, 6373,  6977,  7001, 15329, 16649, 6053, 7211};
  ...
  unsigned hashString(string &s) {
    ...
    for (char c : s) {
      ...
      hashVal = (hashVal +
                 (primeNum[c - 'a'] * appearMap[c] * primeNum[c - 'a']) % mod) %
                mod;
    }
    return hashVal;
  }
