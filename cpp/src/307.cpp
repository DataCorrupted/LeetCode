...
public:
	SegNode(vector<int>& nums, int i, int j):
		left(i), right(j)
	{
		if (i == j){
			val = nums[i];
	...
	int sumRange(int i, int j){
		...
		if (i == left && j == right){ return val; }
		if (i > leftChild->getRight()) { return rightChild->sumRange(i, j); }
		if (j < rightChild->getLeft()) { return leftChild->sumRange(i, j); }
