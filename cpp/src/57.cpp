...
class Solution {
private: 
	bool intervalLe(Interval& lhs, Interval& rhs) { return lhs.start < rhs.start; }
	...
	int binarySearch(vector<Interval>& intervals, Interval target, int l, int r, bool (Solution::*cmpFunc)(Interval&, Interval&)){

		// However, there can be cases where only T or only F are present.
		// Test if the first is F, if so, return not found.
		if (!(this->*cmpFunc)(intervals[l], target)) { return l-1; }
		// Test if the last is T, if so, return the last one.
		if ((this->*cmpFunc)(intervals[r],target)) { return r; }
...
public:
	...
	vector<Interval> insert(vector<Interval>& intervals, Interval newInterval) {
		...
		int idx = binarySearch(intervals, newInterval, 0, intervals.size()-1, &Solution::intervalLe);
...
REGISTER_TEST(example_2){
	...
	vector<Interval> groundTruth = {Interval(1,2),Interval(3,10),Interval(12,16)};
	return Solution().insert(intervals, newInterval) == groundTruth;
}
...
REGISTER_TEST(example_4){
	...
	vector<Interval> groundTruth = {Interval(1,2),Interval(3,5),Interval(6,7),Interval(8,10),Interval(12,17)};
	return Solution().insert(intervals, newInterval) == groundTruth;
}
#endif