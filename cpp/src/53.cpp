...
private:
	...
	std::pair<int, MaxMinPair> divAndConquer(int l, int r){
		if (l == r){
			...
			return std::pair<int, MaxMinPair>(m_nums[l], MaxMinPair(m_subTotal[l], m_subTotal[r]));
		};
		...
		return std::pair<int, MaxMinPair>(
			...
			max(max(leftPart.first, rightPart.first), int(rightPart.second.first - leftPart.second.second)), 
			// Update the max and min data.
			MaxMinPair(
				max(leftPart.second.first, rightPart.second.first),
...
public:
	int maxSubArray(vector<int>& nums) {
		...
		m_nums = nums;
		// Add a dummy head to make sure the first element's sub total is considered.
